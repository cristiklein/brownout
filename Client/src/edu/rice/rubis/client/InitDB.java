...
/**
 * This program initializes the RUBiS database according to the rubis.properties
...
public class InitDB
{
  ...
  /**
   * Main program accepts any combination of the following arguments:
   * 
   * <pre>
  ...
  {
    System.out
        .println("RUBiS database initialization - (C) Rice University/INRIA 2001\n");
    ...
    {
      System.out
          .println("Command line  : java -classpath .:./database edu.rice.rubis.client.InitDB parameters");
      System.out.println("Using Makefile: make initDB PARAM=\"parameters\"");
      System.out
          .println("where parameter is one or any combination of the following arguments:");
      ...
      System.out
          .println(" bids: generate bids and items (it is not possible to create bids without creating the related items)");
      System.out
          .println(" comments: generate comments and items (it is not possible to create comments without creating the related items)");
      Runtime.getRuntime().exit(1);
    }
    ...
    if ((params.indexOf("items") != -1) || (params.indexOf("bids") != -1)
      ...
      initDB.generateItems((params.indexOf("bids") != -1)
          || (params.indexOf("all") != -1), (params.indexOf("comments") != -1)
          || (params.indexOf("all") != -1));
  }
  ...
  public void generateUsers()
  {
    ...
    for (i = 0; i < getNbOfUsers; i++)
    {
      ...
      url = urlGen.registerUser(firstname, lastname, nickname, email, password,
          regionName);
      HTTPreply = callHTTPServer(url);
      if (HTTPreply.indexOf("ERROR") != -1)
      {
        System.err.println("Failed to add user " + firstname + "|" + lastname
            + "|" + nickname + "|" + email + "|" + password + "|" + regionName);
  ...
  public void generateItems(boolean generateBids, boolean generateComments)
  {
    ...
    String[] staticComment = {
        "This is a very bad comment. Stay away from this seller !!<p>",
    ...
    System.out.println("Generating " + oldItems + " old items and "
        + activeItems + " active items.");
    if (generateBids)
      System.out.println("Generating up to " + getMaxBidsPerItem
          + " bids per item.");
    if (generateComments)
    ...
    for (i = 0; i < totalItems; i++)
    {
      ...
      if (i < oldItems)
      { // This is an old item
        duration = -duration; // give a negative auction duration so that
      ...
      url = urlGen.registerItem(name, description, initialPrice, reservePrice,
          buyNow, duration, quantity, sellerId, categoryId + 1);
      HTTPreply = callHTTPServer(url);
      if (HTTPreply.indexOf("ERROR") != -1)
      {
      ...
      { // Now deal with the bids
        ...
        for (j = 0; j < nbBids; j++)
        {
          ...
          url = urlGen.storeBid(i + 1, rand.nextInt(getNbOfUsers) + 1,
              initialPrice, initialPrice + addBid, initialPrice + addBid * 2,
              rand.nextInt(quantity) + 1, quantity);
          HTTPreply = callHTTPServer(url);
          if (HTTPreply.indexOf("ERROR") != -1)
          {
      ...
      { // Generate the comment
        ...
        url = urlGen.storeComment(i + 1, sellerId,
            rand.nextInt(getNbOfUsers) + 1, ratingValue[rating], comment);
        HTTPreply = callHTTPServer(url);
        if (HTTPreply.indexOf("ERROR") != -1)
        {
  ...
  private String callHTTPServer(URL url)
  {
    ...
    while (retry < 5)
    {
      ...
      {
        System.err.println("Unable to read from URL " + url + " ("
            + ioe.getMessage() + ")");
        return null;
      }
